...
const SignUp = () => {
  ...
  const handleSignUp = (e) => {
    ...
    if (
      ...
      setUpError("Use minimum 1 upper & 1 lower case in password");
      return;
    }
    if (!/^(?=.*[A-Z]).+$/.test(password)) {
      setUpError("Use minimum 1 upper case in password");
      return;
    }
    if (!/^(?=.*[a-z]).+$/.test(password)) {
      setUpError("Use minimum 1 lower case in password");
      return;
    }
    ...
    signUpUser(email, password)
      ...
      .catch((error) => {
        ...
        if (error.message == "Firebase: Error (auth/email-already-in-use).") {
          setUpError("Email already in use");
          return;
        }
